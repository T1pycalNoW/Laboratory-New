...
namespace GogoGaga.OptimizedRopesAndCables
{
    ...
    public class Rope : MonoBehaviour
    {
        ...
        public GameObject GetOppositeCircuitComponent(GameObject object1)
        {
            if (startPoint == object1)
            {
                return endPoint.parent.parent.gameObject;
            }
            else if (endPoint == object1)
            {
                return startPoint.parent.parent.gameObject;
            }
            else
            {
                Debug.Log("Неправильно переданный объект!");
                return null;
            }
        }
    }
